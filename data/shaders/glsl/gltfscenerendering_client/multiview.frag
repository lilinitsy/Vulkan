...
float point_light_specular(PointLight light, vec3 normal, vec3 model_position,
						   vec3 view_direction)
{
	float ambient  = 0.1;
	float distance = length(light.position - model_position);
	...
	float spec			 = 0.5 *
				 clamp(pow(max(dot(R, view_direction), 0.0), 16.0), 0.0, 0.25) *
				 distance;
...
void main()
{
	...
	pointlights[0].position = vec3(-8.2, 0, 7.25);
	pointlights[1].position = vec3(-6.2, 0.25, 7.25);
	pointlights[2].position = vec3(-4.2, 0.5, 7.25);
	pointlights[3].position = vec3(-2.2, 0.75, 7.25);

	pointlights[0].diffuse = vec3(0.5, 0.5, 0.5);
	pointlights[1].diffuse = vec3(0.5, 0.5, 0.5);
	pointlights[2].diffuse = vec3(0.5, 0.5, 0.5);
	pointlights[3].diffuse = vec3(0.5, 0.5, 0.5);
	...
	mat3 TBN = mat3(T, B, N);
	vec3 L	 = normalize(inLightVec) * 0.25;
	...
	for(int i = 0; i < 4; i++)
	{
		...
		spec += point_light_specular(pointlights[i], N, inModelPos, V);
	}
